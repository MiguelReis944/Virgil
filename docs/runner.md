...
Set limits in the panel's **Protections** page, restart the core, and inspect the result in **Executions**. See [the local beta pilot](pilots/local-beta.md) for a reproducible test and the evidence to collect. The current gateway accepts OpenAI-compatible chat completions; it does not expose an inbound Anthropic Messages route.
