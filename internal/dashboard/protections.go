...
const protectionsBody = `{{define "content"}}{{if .Error}}<div class="form-err">{{.Error}}</div>{{end}}<form method="post" action="/dashboard/protections"><input type="hidden" name="csrf_token" value="{{.CSRF}}"><div class="cards">
