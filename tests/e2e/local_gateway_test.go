...
// TestThreeErrorBlocking proves that three consecutive identical errors block
// a fourth request for the same run.
func TestThreeErrorBlocking(t *testing.T) {
	callN := 0
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		callN++
		// Return a tool call result every time so the gateway records an error.
		w.Header().Set("Content-Type", "application/json")
		w.Write([]byte(`{"id":"chatcmpl_err","object":"chat.completion","model":"fixture-model","choices":[{"index":0,"message":{"role":"assistant","tool_calls":[{"id":"call1","type":"function","function":{"name":"mytool","arguments":"{}"}}]},"finish_reason":"tool_calls"}],"usage":{"prompt_tokens":5,"completion_tokens":3}}`))
	}))
	defer upstream.Close()
	...
	t.Cleanup(func() { journal.Close(); db.Close() })
	deps := gateway.Dependencies{
		DB: db, Client: http.DefaultClient,
	...
	runID := "run_e2e_blocking_test"
	// Submit three tool-results with the same error to the gateway's /v1/tool-results.
	for i := 0; i < 3; i++ {
		body := fmt.Sprintf(`{"run_id":%q,"tool_results":[{"tool_call_id":"call1","role":"tool","content":"error: tool failed","is_error":true}]}`, runID)
		req := httptest.NewRequest(http.MethodPost, "/v1/tool-results", strings.NewReader(body))
		rec := httptest.NewRecorder()
		handler.ServeHTTP(rec, req)
		// 200 or 204 are acceptable for tool-results submission
	}

	// Fourth request for the same run should be blocked by the policy engine.
	body := fmt.Sprintf(`{"model":"fixture-model","max_tokens":16,"messages":[{"role":"user","content":"hi"}],"run_id":%q}`, runID)
	req := httptest.NewRequest(http.MethodPost, "/v1/chat/completions", strings.NewReader(body))
	req.Header.Set("Authorization", "Bearer test-key")
	rec := httptest.NewRecorder()
	handler.ServeHTTP(rec, req)
...
// mustOpenJournal creates a throw-away journal for policy engine tests.
func mustOpenJournal(t *testing.T) *storage.Journal {
	t.Helper()
