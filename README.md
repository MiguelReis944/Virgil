...
The panel includes Providers, Protections, Executions, Health, Settings, and Usage. Start with Providers and Protections, then launch a supervised child and inspect its result under Executions. [Use one installation with any project or harness](docs/integrations/any-project.md); see the [runner guide](docs/runner.md), [Codex](docs/integrations/codex.md), [Claude Code](docs/integrations/claude-code.md), [desktop access](docs/integrations/desktop.md), and [OmniRoute as upstream](docs/integrations/omniroute.md).
...
The local HTTP server gives request headers five seconds and the entire request read fifteen seconds. Chat request bodies are limited to 1 MiB; a body that stalls returns a generic 408 response and one above the limit returns 413. The server does not set a global write deadline because it streams provider SSE frames. On process interruption, active request contexts and upstream streams are cancelled; graceful shutdown waits up to five seconds before closing remaining connections. A configured provider `base_url` is trusted local configuration and receives the provider bearer token, so use only endpoints you trust.

## Historical proposal and deferred context

The following proposal was written on 2026-09-16 and describes an earlier product vision. Its hosted Control Plane, team dashboard, remote policy, enrollment, and commercial-service sections are retained for history; they are not required for local use and do not describe current capabilities.

The current local gateway remains useful without a hosted service. Secure LAN access and remote governance require separate designs and are deferred.

---

## Historical product proposal: why Virgil exists
